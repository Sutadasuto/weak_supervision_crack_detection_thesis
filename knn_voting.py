...
def main(args):
    ...
    for path in np.transpose(paths):
        ...
        prediction = prediction[:or_shape[0], :or_shape[1], :]  # Remove padding necessary to keep the same size at input and output after encoding

        pixels = np.reshape(prediction, (prediction.shape[0] * prediction.shape[1], prediction.shape[2]))
        labels = get_gt_image(gt_path)[:or_shape[0], :or_shape[1], :].ravel()
...
def parse_args(args=None):
    ...
    parser.add_argument("--save_to", type=str, default=None,
                        help="Results will be saved in this location. If not provided, a folder 'results_dataset-name_"
                             "k-nn' will be created for this purpose.")
