...
from callbacks_and_losses.custom_calllbacks import CorrectedEarlyStoppingAtMinValLoss, ReduceLROnPlateau, TensorBoard
...
def main(args):
    ...
    corrected_paths = data.create_image_paths(args.dataset_names, args.corrected_dataset_paths)
    ...
    n_training_images = int(args.training_split_ratio * all_paths.shape[1])
    np.random.seed(0)
    order = np.array([i for i in range(all_paths.shape[-1])])
    np.random.shuffle(order)
    all_paths = all_paths[:, order]

    training_paths = all_paths[:2, :n_training_images]
    corrected_training_paths = all_paths[2:, :n_training_images]
    validation_paths = all_paths[:2, n_training_images:]
    corrected_validation_paths = all_paths[2:, n_training_images:]
    if args.clean_dataset_paths is not None:
    ...
    while True:
        ...
        # model.compile(optimizer=Adam(lr=args.learning_rate), loss=custom_losses.test_bce_dsc_loss(args.alpha),
        #               metrics=[custom_losses.test_dice_coef, custom_losses.test_bce])
        model.compile(optimizer=Adam(lr=args.learning_rate), loss=custom_losses.corrected_bce_dsc_loss(args.alpha),
                      metrics=[custom_losses.corrected_dice_coef, custom_losses.corrected_bce,
                               custom_losses.corrected_precision, custom_losses.corrected_recall])
        ...
        es = CorrectedEarlyStoppingAtMinValLoss(validation_paths, corrected_validation_paths,
                                       file_path='%s_best.hdf5' % args.model, patience=args.patience,
        ...
        print("Start!")
        try:
            history = model.fit(x=data.corrected_train_image_generator(training_paths, corrected_training_paths,
    ...
    if args.epochs > 0:
        os.replace('%s_best.hdf5' % args.model,
                   os.path.join(results_train_min_loss_dir, '%s_best.hdf5' % args.model))

    # If trained more than one epoch, save the training history as csv and plot it
    if args.epochs > 1:
        print("\nPlotting training history...")
        import pandas as pd
        pd.DataFrame.from_dict(history.history).to_csv(os.path.join(results_dir, "training_history.csv"),
                                                       index=False)
...
def parse_args(args=None):
    ...
    parser.add_argument("-p", "--dataset_paths", type=str, nargs="+",
                        help="Path to the folders or files containing the respective datasets as downloaded from the "
                             "original source.")
    parser.add_argument("-pc", "--corrected_dataset_paths", type=str, nargs="+",
                        help="Path to the folders or files containing the respective datasets with corrected labels.")
