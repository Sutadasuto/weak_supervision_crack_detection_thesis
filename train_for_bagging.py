...
def main(args):
    ...
    results_dir = "results_train_bagging_models_%s" % start if not args.save_to else args.save_to
    ...
    if args.clean_dataset_paths is not None:
        ...
        clean_training_paths = clean_paths[:, :n_training_images]
    ...
    for train_index, validation_index in kf.split(whole_training_paths[0, :]):
        ...
        if args.clean_dataset_paths is not None:
            clean_validation_paths = clean_training_paths[:, validation_index]
        ...
        if args.clean_dataset_paths is not None:
            results_clean_validation_min_loss_dir = os.path.join(results_dir, str(iteration), "results_clean")
        ...
        if args.epochs > 1:
            ...
            import pandas as pd
            pd.DataFrame.from_dict(history.history).to_csv(os.path.join(results_dir, str(iteration), "training_history.csv"),
                                                           index=False)
...
def parse_args(args=None):
    ...
    parser.add_argument("--save_to", type=str, default=None,
                        help="Results will be saved in this location. If not provided, a folder "
                             "'results_train_bagging_models_date_hour' will be created for this purpose.")
