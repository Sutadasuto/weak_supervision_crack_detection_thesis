...
def main(args):
    ...
    for i, path in enumerate(paths.transpose()):
        n_stars = round(bar_size * (i + 1) / paths.shape[-1])
        print("\r[" + "*" * n_stars + " " * (bar_size - n_stars) + "]", end='')

        [im, gt, pred] = data.test_image_from_path(model, path[0], None, rgb_preprocessor)

        or_shape = cv2.imread(path[0], cv2.IMREAD_GRAYSCALE).shape
        pred = pred[:or_shape[0], :or_shape[1]]  # Remove padding necessary to keep the same size at input and output after encoding
        name = os.path.split(path[1])[-1]
        name, extension = os.path.splitext(name)
        extension = ".png"
        if not os.path.exists(args.save_to) and path[1] != '':
